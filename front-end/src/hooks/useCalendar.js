...
const monthNamesArr =[
    ...
    'Mai', 'Juin', 'Juillet', 'Aout', 'Sepembre',
...
const useCalendar = (daysShort = daysShortArr, monthNames = monthNamesArr) =>{
    ...
    const selected = new Date(selectedDate.getFullYear(), selectedDate.getMonth() + 1, 0);
    ...
    for(let i = 1; i < rows + 1; i++){
        for(let j = 1; j < cols + 1; j++){
            ...
            if(i === 1){
                ...
                calendarRows[i] = [...calendarRows[i],{
                classes: '',
                date: `${nextMonthCounter}-${selectedDate.getMonth() + 1}-${selectedDate.getFullYear}`,
                value: currentMonthCounter
            }];
            currentMonthCounter++;
            }
